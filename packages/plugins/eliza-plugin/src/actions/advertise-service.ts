...
import { ServiceOffering } from '../types.js';

export const advertiseServiceAction: Action = {
  ...
  handler: async (
    ...
    for (const service of services) {
      try {
        const response = await tapClient.request<{ serviceId: string }>(
          'POST',
          `${registryUrl}/services/register`,
          {
