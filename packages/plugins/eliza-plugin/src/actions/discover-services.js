...
export const discoverServicesAction = {
    ...
    handler: async (runtime, message, state, options, callback) => {
        ...
        try {
            const response = await tapClient.request('GET', `${registryUrl}/services/discover`, undefined, {
                query,
                category,
                maxPrice,
                minReputation,
            ...
            const serviceList = services
                .map((s) => `- ${s.name} (${s.category}): ${s.pricePerCall} SOL, reputation: ${s.reputationScore}, ${s.verified ? 'verified' : 'unverified'}`)
                .join('\n');
            await callback({
...
//# sourceMappingURL=discover-services.js.map