...
export async function paymentRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/verify', {
    ...
    const { signature, expectedAmount, recipient } = request.body as any;
  ...
  fastify.post('/route', {
    ...
    const { from, to, amount, asset } = request.body as any;
    ...
    await fastify.redis.set(
      `route:${routeId}`,
      JSON.stringify({ from, to, amount, asset, status: 'pending', createdAt: Date.now() }),
      3600
    );
  ...
  fastify.post('/notify', {
    ...
    const { webhookUrl, event, data } = request.body as any;
