...
export const webhookConfigRoutes = async (fastify) => {
    fastify.post('/api/webhooks/config', {
    ...
    fastify.get('/api/webhooks/config', {
        preHandler: [fastify.auth]
    }, async (request, reply) => {
        const authReq = request;
        if (!authReq.wallet) {
    ...
    fastify.put('/api/webhooks/config', {
        preHandler: [fastify.auth],
        schema: {
            body: {
                type: 'object',
    ...
    fastify.delete('/api/webhooks/config', {
        preHandler: [fastify.auth]
    }, async (request, reply) => {
        const authReq = request;
        if (!authReq.wallet) {
    ...
    fastify.post('/api/webhooks/test', {
        preHandler: [fastify.auth]
    }, async (request, reply) => {
        const authReq = request;
        if (!authReq.wallet) {
