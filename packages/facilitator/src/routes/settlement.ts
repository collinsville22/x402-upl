...
export const settlementRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  try {
    ...
    if (treasuryKey && treasuryKey !== '[]') {
      treasuryKeypair = Keypair.fromSecretKey(
        Buffer.from(JSON.parse(treasuryKey))
      );
    }
  } catch (error) {
  ...
  fastify.post<{ Body: SettlementRequest }>(
  ...
  fastify.get<{ Querystring: { merchantWallet: string } }>(
  ...
  fastify.get<{ Querystring: { merchantWallet: string; limit?: number } }>(
  ...
  fastify.post<{ Body: { settlementId: string; reason: string } }>(
    '/api/settlement/cancel',
    {
      preHandler: [(fastify as any).auth],
      schema: {
        body: {
