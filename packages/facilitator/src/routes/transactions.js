...
export const transactionRoutes = async (fastify) => {
    fastify.post('/api/transactions/record', {
        ...
        const { signature, amount, token, senderAddress, recipientAddress, serviceId, agentId, status, timestamp, } = request.body;
        try {
    ...
    fastify.get('/api/transactions/:signature', {
        schema: {
            params: {
                type: 'object',
                required: ['signature'],
