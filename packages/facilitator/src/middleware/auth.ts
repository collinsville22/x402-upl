...
export async function verifyWalletSignature(
  ...
  try {
    ...
    (request as AuthenticatedRequest).wallet = walletAddress;
  } catch (error) {
    return reply.status(401).send({
...
export async function verifyApiKey(
  ...
  try {
    ...
    (request as AuthenticatedRequest).wallet = apiKeyRecord.merchantWallet;
    (request as AuthenticatedRequest).permissions = apiKeyRecord.permissions;
  } catch (error) {
