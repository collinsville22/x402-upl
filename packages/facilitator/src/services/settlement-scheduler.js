...
export class SettlementScheduler {
    ...
    constructor(logger) {
        ...
        this.treasuryKeypair = Keypair.fromSecretKey(Buffer.from(JSON.parse(process.env.TREASURY_PRIVATE_KEY || '[]')));
        this.tokenMint = new PublicKey(process.env.TOKEN_MINT || '4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU');
    ...
    async processScheduledSettlement(config) {
        ...
        try {
            const result = await this.executeSettlement(config.merchantWallet, config.serviceId, transactions.map(tx => tx.id));
    ...
    async executeSettlement(merchantWallet, serviceId, transactionIds) {
        const transactions = await prisma.transaction.findMany({
            where: {
    ...
    async transferTokens(merchantWallet, amount) {
        const merchantPubkey = new PublicKey(merchantWallet);
        const treasuryTokenAccount = await getAssociatedTokenAddress(this.tokenMint, this.treasuryKeypair.publicKey);
        const merchantTokenAccount = await getAssociatedTokenAddress(this.tokenMint, merchantPubkey);
        ...
        const transaction = new Transaction().add(createTransferInstruction(treasuryTokenAccount, merchantTokenAccount, this.treasuryKeypair.publicKey, amountInTokens, [], TOKEN_PROGRAM_ID));
    ...
    async notifyMerchant(merchantWallet, result) {
        ...
        await webhookService.enqueueWebhook(service.webhookUrl, 'settlement.completed', {
    ...
    removeSchedule(serviceId, merchantWallet) {
