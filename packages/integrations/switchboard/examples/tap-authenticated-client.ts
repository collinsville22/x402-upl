...
async function main() {
  ...
  if (btcFeed) {
    ...
    const simulateResponse = await axios.post(
      `${marketplaceUrl}/feeds/${btcFeed.feedId}/simulate`,
      simulateBody,
  ...
  const createHeaders = await tapAgent.signRequest(
    'POST',
    '/feeds/custom',
    customFeedRequest
  );
  ...
  const batchRequest = {
    feedIds: feedsResponse.data.slice(0, 3).map((f: any) => f.feedId),
  };

  const batchHeaders = await tapAgent.signRequest(
    'POST',
  ...
  const batchResponse = await axios.post(
    `${marketplaceUrl}/feeds/batch/simulate`,
    batchRequest,
    { headers: batchHeaders }
  );

  console.log(`Simulated ${Object.keys(batchResponse.data).length} feeds:`);
  for (const [feedId, result] of Object.entries(batchResponse.data as any)) {
    const feed = feedsResponse.data.find((f: any) => f.feedId === feedId);
  ...
  const servicesResponse = await axios.get(
    `${marketplaceUrl}/marketplace/services?category=crypto-price`
  );
