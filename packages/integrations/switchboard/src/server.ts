...
export class SwitchboardMarketplaceServer {
  ...
  constructor(config: ServerConfig) {
    ...
    this.marketplace = new OracleDataMarketplace(
      config.rpcUrl,
      config.queueKey,
      config.paymentRecipient,
      config.x402RegistryUrl,
      config.network,
      config.redisUrl
    );
  ...
  private setupRoutes(): void {
    ...
    this.server.post('/feeds/custom', async (request, reply) => {
      ...
      try {
        const customRequest = request.body as CustomFeedRequest;
        const feed = await this.marketplace.createCustomFeed(
          customRequest,
          tapResult.identity!.keyId
        );
    ...
    this.server.post('/feeds/batch/simulate', async (request, reply) => {
      ...
      const { feedIds } = request.body as { feedIds: string[] };
    ...
    this.server.get('/marketplace/services', async (request, reply) => {
      const { category, maxPrice } = request.query as { category?: string; maxPrice?: string };

      try {
        const services = await this.marketplace.discoverMarketplaceServices(
          category,
  ...
  private async handleFeedUpdate(
    ...
    if (!paymentProofHeader) {
      const requirement = await this.marketplace.requestFeedUpdate(
        feedId,
        tapResult.identity!.keyId
  ...
  private async handleWebSocketFeed(connection: any, request: any): Promise<void> {
  ...
  private async verifyTAPSignature(
    request: FastifyRequest
    ...
    return await this.tapVerifier.verifySignature(
      request.method,
      request.url,
  ...
  async start(): Promise<void> {
    try {
      ...
      this.server.log.info(
        `Switchboard Oracle Marketplace running on ${this.config.host}:${this.config.port}`
      );
