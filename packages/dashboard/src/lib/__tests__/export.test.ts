import {
  ...
  formatSettlementsForExport,
  generateFilename,
} from '../export';
import type { Transaction, Settlement } from '../api';
...
describe('Export Utilities', () => {
  ...
  describe('formatSettlementsForExport', () => {
    it('should format settlements correctly', () => {
      ...
      const formatted = formatSettlementsForExport(settlements);
  ...
  describe('generateFilename', () => {
    ...
    it('should generate JSON filename with timestamp', () => {
      const filename = generateFilename('settlements', 'json');

      expect(filename).toMatch(/^settlements_2025-11-12_\d{2}-\d{2}-\d{2}\.json$/);
    });

    it('should use provided prefix', () => {
      const filename = generateFilename('custom-prefix', 'csv');
