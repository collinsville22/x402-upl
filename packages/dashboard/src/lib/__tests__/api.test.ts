import { FacilitatorAPI } from '../api';
...
describe('FacilitatorAPI', () => {
  ...
  beforeEach(() => {
    ...
    api = new FacilitatorAPI('http://localhost:4001');
  });
  ...
  describe('getTransactions', () => {
    it('should fetch transactions with query parameters', async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        'http://localhost:4001/api/transactions?agentId=agent1&limit=10'
      );
  ...
  describe('getPendingSettlement', () => {
    it('should fetch pending settlement for wallet', async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        'http://localhost:4001/api/settlement/pending?merchantWallet=wallet1'
      );
  ...
  describe('requestSettlement', () => {
    it('should request settlement with correct payload', async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        'http://localhost:4001/api/settlement/request',
        {
          method: 'POST',
          headers: {
  ...
  describe('getSettlementHistory', () => {
    it('should fetch settlement history with limit', async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        'http://localhost:4001/api/settlement/history?merchantWallet=wallet1&limit=50'
      );
  ...
  describe('searchServices', () => {
    ...
    it('should handle empty search query', async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        'http://localhost:4001/api/services/search?'
      );
    });
  });
  ...
  describe('getServiceRecommendations', () => {
    it('should fetch service recommendations', async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        'http://localhost:4001/api/services/service1/recommendations?limit=5'
      );
  ...
  describe('getServiceReputation', () => {
    it('should fetch service reputation', async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        'http://localhost:4001/api/services/service1/reputation'
      );
  ...
  describe('getTrendingServices', () => {
    it('should fetch trending services', async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        'http://localhost:4001/api/services/trending?limit=10'
      );
