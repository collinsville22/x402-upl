...
export default function ServicesPage() {
  ...
  const { data: stats, isLoading: statsLoading } = useQuery({
    ...
    queryFn: async () => {
      if (!wallet || !allServices) return {};

      const merchantServices = allServices.filter(s => s.wallet === wallet);
      const statsPromises = merchantServices.map(service =>
        facilitatorAPI.getServiceStats(service.id)
      );
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="ml-4 flex flex-col gap-2">
                    <Link
                      ...
                      className="rounded-lg border border-slate-300 px-3 py-1 text-center text-sm text-slate-700 hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700"
                    >
                      Manage
                    </Link>
                    <Link
