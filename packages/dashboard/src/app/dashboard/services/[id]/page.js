...
exports.default = ServiceManagePage;
const react_query_1 = require("@tanstack/react-query");
const api_1 = require("@/lib/api");
const navigation_1 = require("next/navigation");
const react_1 = require("react");
function ServiceManagePage() {
    ...
    const { data: transactions, isLoading: txLoading } = (0, react_query_1.useQuery)({
        ...
        queryFn: async () => {
            return await api_1.facilitatorAPI.getTransactions({ serviceId, limit: 50 });
        },
        enabled: !!serviceId,
    });
    ...
    return (<div className="space-y-6">
      <div>
        <button onClick={() => router.push('/dashboard/services')} className="mb-4 text-sm text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white">
          ← Back to Services
        </button>
      ...
      <div className="rounded-lg bg-white p-6 shadow dark:bg-slate-800">
        ...
        <div className="mt-4 space-y-4">
          <div>
            ...
            <input type="text" value={service.url} disabled className="mt-1 w-full rounded-lg border border-slate-300 bg-slate-50 px-3 py-2 text-slate-900 dark:border-slate-600 dark:bg-slate-700 dark:text-white"/>
          </div>
          <div>
            <label className="text-sm font-medium text-slate-700 dark:text-slate-300">
              Wallet Address
            </label>
            <input type="text" value={service.wallet} disabled className="mt-1 w-full rounded-lg border border-slate-300 bg-slate-50 px-3 py-2 text-slate-900 dark:border-slate-600 dark:bg-slate-700 dark:text-white"/>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <input type="text" value={service.category || 'Uncategorized'} disabled className="mt-1 w-full rounded-lg border border-slate-300 bg-slate-50 px-3 py-2 text-slate-900 dark:border-slate-600 dark:bg-slate-700 dark:text-white"/>
            </div>
            <div>
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300">
                Price per Call
              </label>
              <input type="text" value={`${parseFloat(service.pricePerCall).toFixed(2)} USDC`} disabled className="mt-1 w-full rounded-lg border border-slate-300 bg-slate-50 px-3 py-2 text-slate-900 dark:border-slate-600 dark:bg-slate-700 dark:text-white"/>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-lg bg-white shadow dark:bg-slate-800">
        ...
        {transactions && transactions.length > 0 ? (<div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                {transactions.slice(0, 10).map((tx) => (<tr key={tx.id}>
          ...
          </div>) : (<div className="flex h-32 items-center justify-center">
