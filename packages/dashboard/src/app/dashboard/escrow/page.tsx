...
interface EscrowTransaction {
  id: string;
  type: 'deposit' | 'withdrawal' | 'workflow_payment' | 'workflow_refund';
  amount: number;
  signature: string;
  workflowId?: string;
  timestamp: string;
...
export default function EscrowPage() {
  ...
  const [history, setHistory] = useState<EscrowTransaction[]>([]);
    ...
    async function fetchData() {
      ...
      try {
        ...
        const infoResponse = await fetch(
          `${process.env.NEXT_PUBLIC_REASONING_URL || 'http://localhost:5000'}/escrow/address`
        );
        if (infoResponse.ok) {
        ...
        const historyResponse = await fetch(
          `${process.env.NEXT_PUBLIC_REASONING_URL || 'http://localhost:5000'}/escrow/${wallet.toString()}/history?limit=50`
        );
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_REASONING_URL || 'http://localhost:5000'}/escrow/${wallet.toString()}/withdraw`,
        {
          method: 'POST',
          headers: {
        ...
        {loading ? (
          ...
          <>
            ...
            {escrowInfo && (
              <div className="rounded-lg border border-[#2A2A2A] bg-[#111111] p-6">
                ...
                <div className="mt-3 flex items-center gap-3">
                  ...
                  <button
                    onClick={() => navigator.clipboard.writeText(escrowInfo.address)}
                    className="rounded-lg border border-[#2A2A2A] px-4 py-2 text-sm text-[#888888] transition-colors hover:bg-[#1A1A1A] hover:text-white"
                  >
                    Copy
                  </button>
                </div>
                ...
                <div className="mt-4 space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      step="0.01"
                      min="0.01"
                      className="mt-2 block w-full rounded-lg border border-[#2A2A2A] bg-[#0A0A0A] px-4 py-3 text-white placeholder-[#555555] transition-colors focus:border-[#00FF88] focus:outline-none focus:ring-1 focus:ring-[#00FF88]"
                      placeholder="0.1"
                    />
                  </div>
                  <button
                    ...
                    disabled={depositing}
                    className="w-full rounded-lg bg-[#00FF88] px-6 py-3 font-semibold text-[#0A0A0A] transition-all hover:bg-[#00DD77] disabled:opacity-50"
                  >
              ...
              <div className="rounded-lg border border-[#2A2A2A] bg-[#111111] p-6">
                ...
                <div className="mt-4 space-y-4">
                  ...
                  <button
                    ...
                    disabled={withdrawing || !balance || parseFloat(withdrawAmount) > balance.available}
                    className="w-full rounded-lg border border-[#00FF88] px-6 py-3 font-semibold text-[#00FF88] transition-all hover:bg-[#00FF88] hover:text-[#0A0A0A] disabled:opacity-50"
                  >
            ...
            <div className="rounded-lg border border-[#2A2A2A] bg-[#111111] p-6">
              ...
              {history.length === 0 ? (
                <p className="mt-4 text-center text-sm text-[#888888]">No transactions yet</p>
              ) : (
                  ...
                  {history.map((tx) => (
                    ...
                    >
                      <div>
                        ...
                        {tx.workflowId && (
                          <button
                            onClick={() => router.push(`/dashboard/workflows/${tx.workflowId}`)}
