...
function WorkflowDetailPage() {
    ...
    (0, react_1.useEffect)(() => {
        async function fetchWorkflow() {
            try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_REASONING_URL || 'http://localhost:5000'}/workflows/${id}`);
                if (!response.ok) {
    ...
    async function handleApprove() {
        setApproving(true);
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_REASONING_URL || 'http://localhost:5000'}/workflows/${id}/approve`, { method: 'POST' });
            const data = await response.json();
            if (!data.success) {
    ...
    async function handleCancel() {
        setCancelling(true);
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_REASONING_URL || 'http://localhost:5000'}/workflows/${id}/cancel`, { method: 'POST' });
            const data = await response.json();
            if (!data.success) {
    ...
    if (error || !workflow) {
        return (<div className="flex min-h-screen items-center justify-center bg-[#0A0A0A] px-4">
        <div className="text-center">
          <p className="text-lg font-semibold text-red-400">{error || 'Workflow not found'}</p>
          <button onClick={() => router.push('/dashboard/workflows')} className="mt-4 text-sm text-[#00FF88] hover:underline">
            Back to workflows
          </button>
        </div>
      </div>);
    ...
    return (<div className="min-h-screen bg-[#0A0A0A] px-4 py-8">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        {workflow.status === 'awaiting_approval' && workflow.executionPlan && (<div className="rounded-lg border border-yellow-500/20 bg-yellow-500/10 p-6">
            ...
            <div className="mt-4 flex gap-4">
              <button onClick={handleApprove} disabled={approving} className="rounded-lg bg-[#00FF88] px-6 py-2 font-semibold text-[#0A0A0A] transition-all hover:bg-[#00DD77] disabled:opacity-50">
                {approving ? 'Approving...' : 'Approve & Execute'}
              </button>
              <button onClick={handleCancel} disabled={cancelling} className="rounded-lg border border-[#2A2A2A] px-6 py-2 font-semibold text-white transition-colors hover:bg-[#1A1A1A] disabled:opacity-50">
        ...
        {/* Execution Plan */}
        {workflow.executionPlan && (<div className="rounded-lg border border-[#2A2A2A] bg-[#111111] p-6">
            ...
            <div className="mt-4 space-y-3">
              {workflow.executionPlan.steps.map((step, index) => {
                ...
                return (<div key={step.id} className={`flex items-center gap-4 rounded-lg border p-4 transition-all ${stepStatus === 'completed'
                    ...
                    <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full border border-[#2A2A2A] bg-[#111111]">
                      {stepStatus === 'completed' ? (<svg className="h-5 w-5 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7"/>
                        </svg>) : stepStatus === 'failed' ? (<svg className="h-5 w-5 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/>
                        </svg>) : stepStatus === 'executing' ? (<div className="h-4 w-4 animate-spin rounded-full border-2 border-[#2A2A2A] border-t-[#00FF88]"></div>) : (<span className="text-xs text-[#888888]">{index + 1}</span>)}
                    </div>
        ...
        {/* Real-time Events */}
        {events.length > 0 && (<div className="rounded-lg border border-[#2A2A2A] bg-[#111111] p-6">
            <h2 className="text-lg font-semibold text-white">Real-time Events</h2>
            <div className="mt-4 max-h-96 space-y-2 overflow-y-auto">
              {events.map((event, index) => (<div key={index} className="rounded border border-[#2A2A2A] bg-[#0A0A0A] p-3 font-mono text-xs">
                  ...
                  {event.data && (<pre className="mt-2 text-[#CCCCCC]">{JSON.stringify(event.data, null, 2)}</pre>)}
                </div>))}
            </div>
          </div>)}

        {/* Error Display */}
        {workflow.error && (<div className="rounded-lg border border-red-500/20 bg-red-500/10 p-6">
            <h3 className="font-semibold text-red-400">Workflow Error</h3>
            <p className="mt-2 text-sm text-red-300">{workflow.error.message}</p>
            {workflow.error.stepId && (<p className="mt-1 text-xs text-red-400">Step ID: {workflow.error.stepId}</p>)}
          </div>)}

        {/* Action Buttons */}
        <div className="flex gap-4">
          <button onClick={() => router.push('/dashboard/workflows')} className="rounded-lg border border-[#2A2A2A] px-6 py-3 font-semibold text-white transition-colors hover:bg-[#1A1A1A]">
            Back to Workflows
          </button>
          {(workflow.status === 'executing' || workflow.status === 'awaiting_approval') && (<button onClick={handleCancel} disabled={cancelling} className="rounded-lg border border-red-500/20 px-6 py-3 font-semibold text-red-400 transition-colors hover:bg-red-500/10 disabled:opacity-50">
