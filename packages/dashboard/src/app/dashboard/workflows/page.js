...
function WorkflowsPage() {
    ...
    (0, react_1.useEffect)(() => {
        async function fetchWorkflows() {
            ...
            try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_REASONING_URL || 'http://localhost:5000'}/users/${wallet.toString()}/workflows?limit=50`);
                if (response.ok) {
    ...
    return (<div className="min-h-screen bg-[#0A0A0A] px-4 py-8">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="flex items-center justify-between border-b border-[#2A2A2A] pb-6">
          ...
          <button onClick={() => router.push('/dashboard/workflows/new')} className="rounded-lg bg-[#00FF88] px-6 py-3 font-semibold text-[#0A0A0A] transition-all hover:bg-[#00DD77]">
            Create Workflow
          </button>
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto">
          {['all', 'awaiting_approval', 'executing', 'completed', 'failed'].map((status) => (<button key={status} onClick={() => setFilter(status)} className={`whitespace-nowrap rounded-lg border px-4 py-2 text-sm font-medium transition-colors ${filter === status
        ...
        {loading ? (<div className="flex h-64 items-center justify-center">
          ...
          </div>) : filteredWorkflows.length === 0 ? (<div className="flex h-64 items-center justify-center rounded-lg border border-[#2A2A2A] bg-[#111111]">
            <div className="text-center">
              ...
              {filter === 'all' && (<button onClick={() => router.push('/dashboard/workflows/new')} className="mt-4 rounded-lg bg-[#00FF88] px-6 py-2 text-sm font-semibold text-[#0A0A0A] transition-all hover:bg-[#00DD77]">
                  Create Workflow
                </button>)}
            </div>
            ...
            {filteredWorkflows.map((workflow) => {
                ...
                return (<button key={workflow.id} onClick={() => router.push(`/dashboard/workflows/${workflow.id}`)} className="w-full rounded-lg border border-[#2A2A2A] bg-[#111111] p-6 text-left transition-all hover:border-[#00FF88]/30 hover:bg-[#1A1A1A]">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs text-[#888888]">
                          {(0, date_fns_1.format)(new Date(workflow.createdAt), 'MMM dd, yyyy HH:mm')}
                        </span>
                      </div>
        ...
        {/* Stats Summary */}
        {workflows.length > 0 && (<div className="grid gap-6 md:grid-cols-4">
