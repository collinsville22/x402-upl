...
export default function DiscoverPageV2() {
  ...
  const filteredAndSortedServices = useMemo(() => {
    ...
    if (selectedTokens.length > 0) {
      filtered = filtered.filter(s =>
        selectedTokens.some(token => s.acceptedTokens.includes(token))
      );
    }
        ...
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="rounded-lg border border-[#2A2A2A] bg-[#111111] px-4 py-3 text-sm text-white hover:bg-[#1A1A1A]"
            >
              ...
              <div className="rounded-lg border border-[#2A2A2A] bg-[#111111] p-6">
                ...
                <div className="mt-4 space-y-2">
                  {availableTokens.map((token) => (
                    <label
                      key={token}
                      className="flex cursor-pointer items-center gap-3 rounded-lg px-3 py-2 hover:bg-[#1A1A1A]"
                    >
                      <input
                        type="checkbox"
                        checked={selectedTokens.includes(token)}
                        onChange={() => toggleToken(token)}
                        className="h-4 w-4 rounded border-[#2A2A2A] bg-[#0A0A0A] text-[#00FF88] focus:ring-[#00FF88]"
                      />
            ...
            {filteredAndSortedServices.length > 0 ? (
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {filteredAndSortedServices.map((service) => (
                  <button
                    key={service.id}
                    onClick={() => router.push(`/dashboard/discover/${service.id}`)}
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        {service.category && (
                          <span className="mt-2 inline-flex rounded-lg border border-[#00FF88]/20 bg-[#00FF88]/10 px-2 py-1 text-xs font-medium text-[#00FF88]">
                            {service.category}
                          </span>
                        )}
                      </div>
                    ...
                    <div className="mt-4 flex items-center justify-between">
                      ...
                      <div className="flex gap-1">
                        {service.acceptedTokens.slice(0, 3).map((token) => (
                          <span
                            key={token}
