...
type TabType = 'overview' | 'docs' | 'try' | 'reviews' | 'analytics';
        ...
        <div className="border-b border-[#2A2A2A] pb-6">
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              {service.category && (
                <span className="mt-3 inline-flex rounded-lg border border-[#00FF88]/20 bg-[#00FF88]/10 px-3 py-1 text-xs font-semibold text-[#00FF88]">
                  {service.category}
                </span>
              )}
            </div>
              ...
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  <svg
                    key={star}
                    className={`h-4 w-4 ${
                      star <= Math.round(averageRating) ? 'text-yellow-400' : 'text-[#2A2A2A]'
                    }`}
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                  ...
                  <div>
                    ...
                    <div className="mt-2 flex gap-2">
                      {service.acceptedTokens.map((token) => (
                        <span
                          key={token}
                          className="rounded-lg border border-[#2A2A2A] bg-[#0A0A0A] px-3 py-1 text-xs font-medium text-white"
                        >
                          {token}
                        </span>
                      ))}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="mt-2 block w-full rounded-lg border border-[#2A2A2A] bg-[#0A0A0A] px-4 py-3 text-white placeholder-[#555555] focus:border-[#00FF88] focus:outline-none focus:ring-1 focus:ring-[#00FF88]"
                  />
                </div>
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    ...
                    <select
                      ...
                      className="mt-2 block w-full rounded-lg border border-[#2A2A2A] bg-[#0A0A0A] px-4 py-3 text-white focus:border-[#00FF88] focus:outline-none focus:ring-1 focus:ring-[#00FF88]"
                    >
                ...
                {testMethod === 'POST' && (
                  <div>
                    ...
                    <textarea
                      ...
                      rows={6}
                      className="mt-2 block w-full rounded-lg border border-[#2A2A2A] bg-[#0A0A0A] px-4 py-3 font-mono text-sm text-white placeholder-[#555555] focus:border-[#00FF88] focus:outline-none focus:ring-1 focus:ring-[#00FF88]"
                    />
                  </div>
                )}
                <button
                  ...
                  className="rounded-lg bg-[#00FF88] px-6 py-3 font-semibold text-[#0A0A0A] transition-all hover:bg-[#00DD77] disabled:opacity-50"
                >
