...
function AnalyticsPage() {
    ...
    const [workflowStats, setWorkflowStats] = (0, react_1.useState)(null);
    const [spendingTrends, setSpendingTrends] = (0, react_1.useState)([]);
    ...
    (0, react_1.useEffect)(() => {
        async function fetchAnalytics() {
            ...
            try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_REASONING_URL || 'http://localhost:5000'}/users/${wallet.toString()}/workflows?limit=100`);
                if (response.ok) {
                    ...
                    if (data.success && data.workflows) {
                        ...
                        const stats = {
                            ...
                            totalSpent: workflows.reduce((sum, w) => sum + (w.totalCost || 0), 0),
                            averageCost: 0,
                        };
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="flex items-center justify-between border-b border-[#2A2A2A] pb-6">
          ...
          <div className="flex gap-2">
            <button onClick={() => handleExport('csv')} className="rounded-lg border border-[#2A2A2A] bg-[#111111] px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-[#1A1A1A]">
              Export CSV
            </button>
            <button onClick={() => handleExport('json')} className="rounded-lg border border-[#2A2A2A] bg-[#111111] px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-[#1A1A1A]">
              Export JSON
            </button>
          </div>
        </div>
        ...
        {/* Time Range Selector */}
        <div className="flex gap-2">
          {['7d', '30d', '90d'].map((range) => (<button key={range} onClick={() => setTimeRange(range)} className={`rounded-lg border px-4 py-2 text-sm font-medium transition-colors ${timeRange === range
        ...
        {workflowStats ? (<>
            ...
            {/* Spending Trends */}
            {spendingTrends.length > 0 && (<div className="rounded-lg border border-[#2A2A2A] bg-[#111111] p-6">
                ...
                <div className="mt-6 flex h-64 items-end gap-2">
                  {spendingTrends.map((trend, i) => {
                    ...
                    return (<div key={i} className="flex flex-1 flex-col items-center gap-2">
                        <div className="relative w-full">
                          <div className="w-full rounded-t bg-[#00FF88]" style={{ height: `${height}%`, minHeight: trend.amount > 0 ? '4px' : '0' }}/>
                        </div>
                        <div className="text-center">
                ...
                <div className="mt-6 space-y-4">
                  <div>
                    ...
                    <div className="mt-2 h-2 rounded-full bg-[#2A2A2A]">
                      <div className="h-2 rounded-full bg-green-400" style={{
                width: `${workflowStats.totalWorkflows > 0
