      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative flex-1">
            <input type="text" placeholder="Search integrations..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full rounded-lg border border-[#2A2A2A] bg-[#111111] px-4 py-3 pl-10 text-white placeholder-[#555555] focus:border-[#00FF88] focus:outline-none focus:ring-1 focus:ring-[#00FF88]"/>
          ...
          <div className="flex gap-2">
            <select value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)} className="rounded-lg border border-[#2A2A2A] bg-[#111111] px-4 py-3 text-sm text-white focus:border-[#00FF88] focus:outline-none focus:ring-1 focus:ring-[#00FF88]">
        ...
        {/* Integrations Grid */}
        {filteredIntegrations.length > 0 ? (<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filteredIntegrations.map((integration) => (<button key={integration.id} onClick={() => router.push(`/dashboard/integrations/${integration.id}`)} className="rounded-lg border border-[#2A2A2A] bg-[#111111] p-6 text-left transition-all hover:border-[#00FF88]/30 hover:bg-[#1A1A1A]">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-lg" style={{ backgroundColor: integration.color + '20', border: `2px solid ${integration.color}` }}>
                      <div className="h-6 w-6 rounded" style={{ backgroundColor: integration.color }}></div>
                    </div>
                    <div>
                      <h3 className="font-semibold text-white">{integration.name}</h3>
                      <span className={`mt-1 inline-flex rounded-lg border px-2 py-0.5 text-xs font-medium ${integration.status === 'active'
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {integration.capabilities.slice(0, 3).map((capability) => (<span key={capability} className="rounded border border-[#2A2A2A] bg-[#0A0A0A] px-2 py-1 text-xs text-[#CCCCCC]">
                      {capability}
                    </span>))}
                  {integration.capabilities.length > 3 && (<span className="rounded border border-[#2A2A2A] bg-[#0A0A0A] px-2 py-1 text-xs text-[#888888]">
