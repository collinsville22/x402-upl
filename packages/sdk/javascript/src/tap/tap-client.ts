import { RFC9421Signature, SignatureAlgorithm, SignatureParams, SignatureComponents } from './rfc9421.js';
import axios, { AxiosInstance } from 'axios';

export interface TAPConfig {
  keyId: string;
  privateKey: Uint8Array | string;
  algorithm: SignatureAlgorithm;
  registryUrl?: string;
  did?: string;
  visaTapCert?: string;
}

export interface AgentIdentity {
  did: string;
  visaTapCert: string;
  walletAddress: string;
  reputationScore?: number;
}

export class TAPClient {
  private config: TAPConfig;
  private httpClient: AxiosInstance;
  private agentIdentity?: AgentIdentity;

  constructor(config: TAPConfig, agentIdentity?: AgentIdentity) {
    this.config = config;
    this.agentIdentity = agentIdentity;
    this.httpClient = axios.create();
  }

  async signRequest(url: string, method: 'GET' | 'POST' = 'GET'): Promise<Record<string, string>> {
    const urlObj = new URL(url);

    const components: SignatureComponents = {
      authority: urlObj.host,
      path: urlObj.pathname + urlObj.search,
    };

    const now = Math.floor(Date.now() / 1000);
    const params: SignatureParams = {
      created: now,
      expires: now + 300,
      keyId: this.config.keyId,
      alg: this.config.algorithm,
      nonce: RFC9421Signature.generateNonce(),
      tag: 'agent-payer-auth',
    };

    let result;

    if (this.config.algorithm === 'ed25519') {
      if (!(this.config.privateKey instanceof Uint8Array)) {
        throw new Error('Ed25519 requires Uint8Array private key');
      }
      result = await RFC9421Signature.signEd25519(components, params, this.config.privateKey);
    } else {
      if (typeof this.config.privateKey !== 'string') {
        throw new Error('RSA-PSS requires PEM string private key');
      }
      result = await RFC9421Signature.signRsaPss(components, params, this.config.privateKey);
    }

    return {
      'Signature-Input': result.signatureInput,
      'Signature': result.signature,
    };
  }

  async request<T = any>(
    method: 'GET' | 'POST',
    url: string,
    data?: any,
    params?: Record<string, any>
  ): Promise<T> {
    const headers = await this.signRequest(url, method);

    if (this.agentIdentity) {
      headers['X-Agent-DID'] = this.agentIdentity.did;
      headers['X-Agent-Cert'] = this.agentIdentity.visaTapCert;
      headers['X-Agent-Wallet'] = this.agentIdentity.walletAddress;
    }

    const response = await this.httpClient.request({
      method,
      url,
      data,
      params,
      headers,
    });

    return response.data;
  }

  async registerAgent(walletAddress: string, stake?: number): Promise<AgentIdentity> {
    if (!this.config.registryUrl) {
      throw new Error('Registry URL required for agent registration');
    }

    const registrationData = {
      did: this.config.did || `did:x402:${this.config.keyId}`,
      walletAddress,
      visaTapCert: this.config.visaTapCert || this.config.keyId,
      publicKey: this.getPublicKeyString(),
      algorithm: this.config.algorithm,
      stake: stake || 0,
    };

    const response = await this.request<{ agent: AgentIdentity }>(
      'POST',
      `${this.config.registryUrl}/agents/register`,
      registrationData
    );

    this.agentIdentity = response.agent;
    return response.agent;
  }

  async discoverAgents(filters?: {
    category?: string;
    minReputation?: number;
    verified?: boolean;
  }): Promise<AgentIdentity[]> {
    if (!this.config.registryUrl) {
      throw new Error('Registry URL required for agent discovery');
    }

    const response = await this.request<{ agents: AgentIdentity[] }>(
      'GET',
      `${this.config.registryUrl}/agents/discover`,
      undefined,
      filters
    );

    return response.agents;
  }

  getAgentIdentity(): AgentIdentity | undefined {
    return this.agentIdentity;
  }

  private getPublicKeyString(): string {
    if (this.config.algorithm === 'ed25519' && this.config.privateKey instanceof Uint8Array) {
      return Buffer.from(this.config.privateKey).toString('base64');
    }
    return 'public-key-placeholder';
  }
}
