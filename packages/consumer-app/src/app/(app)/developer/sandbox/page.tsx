...
export default function TestingSandboxPage() {
  ...
  async function testEndpoint(endpoint: typeof testEndpoints[0]) {
    if (!wallet) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Info Banner */}
      <Card className="border-l-4 border-x402-accent bg-x402-accent-muted/10 p-6">
      ...
      <Card className="p-6">
        ...
        <div className="mt-4 grid gap-4">
          {testEndpoints.map((endpoint) => (
            <Card key={endpoint.id} variant="hover" className="p-4">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-3">
                    <h3 className="font-medium text-white">{endpoint.name}</h3>
                    <Badge variant="neutral">{endpoint.method}</Badge>
                ...
                <Button
                  ...
                  loading={testing && selectedEndpoint === endpoint.id}
                  disabled={testing}
                >
                  Test
                </Button>
        ...
        <Card className="p-6">
          ...
          <div className="mt-4 space-y-3">
            {testResults.map((result, idx) => (
              <div
                key={idx}
                className="flex items-center justify-between rounded-lg border border-x402-border bg-x402-surface p-4"
              >
                <div className="flex-1">
                  <div className="flex items-center gap-3">
                    ...
                    {result.refundStatus && (
                      <Badge variant="warning">{result.refundStatus}</Badge>
                    )}
                  </div>
                  {result.message && (
                    <p className="mt-1 text-sm text-x402-text-secondary">{result.message}</p>
                  )}
                  {result.error && (
                ...
                {result.cost && (
                  <span className="text-sm font-medium text-x402-text-secondary">
                    {result.cost} SOL
                  </span>
                )}
              </div>
