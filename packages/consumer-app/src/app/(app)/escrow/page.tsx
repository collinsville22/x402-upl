...
interface Transaction {
  id: string;
  type: 'deposit' | 'withdraw' | 'workflow_payment' | 'workflow_refund';
  amount: number;
  workflowId?: string;
  signature?: string;
  timestamp: string;
...
export default function EscrowPage() {
  ...
  const setLoading = useEscrowStore((state) => state.setLoading);
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      const response = await reasoningAPI.depositEscrow(
        wallet.toString(),
        amount,
        mockSignature
      );
  ...
  const handleWithdraw = async () => {
    ...
    try {
      const response = await reasoningAPI.withdrawEscrow(wallet.toString(), amount);
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        ...
        {transactions.length === 0 ? (
          <p className="mt-4 text-center text-sm text-x402-text-tertiary">
            No transactions yet
          </p>
        ) : (
          <div className="mt-4 overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {transactions.map((tx) => (
                  <tr key={tx.id} className="border-b border-x402-border text-sm">
                    ...
                    <td className="py-4">
                      {tx.workflowId ? (
                        <a
                          href={`/workflows/${tx.workflowId}`}
                          className="text-x402-accent hover:underline"
                        >
                          {tx.workflowId.slice(0, 8)}...
                        </a>
                      ) : (
                        <span className="text-x402-text-muted">—</span>
                      )}
                    </td>
