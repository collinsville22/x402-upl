...
function NewWorkflowPage() {
    ...
    const [maxBudget, setMaxBudget] = (0, react_1.useState)(0.1);
    const [maxTime, setMaxTime] = (0, react_1.useState)(300);
    ...
    const handleSubmit = async () => {
        ...
        setError('');
        try {
            const response = await api_1.reasoningAPI.createWorkflow({
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card_1.Card className="p-6">
            ...
            <Textarea_1.Textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="Example: Deploy a new SPL token on Solana with 1M supply and liquidity pool on Raydium..." className="min-h-[200px] font-mono text-sm" error={error}/>
            <div className="mt-2 flex items-center justify-between text-xs">
              <span className="text-x402-text-tertiary">
                Use @ to mention specific integrations (e.g., @coinbase, @switchboard)
              </span>
              <span className={charCount < charMin
            ? 'text-x402-error'
            : charCount > charMax
                ? 'text-x402-error'
          ...
          <Card_1.Card className="p-6">
            ...
            <div className="mt-4 space-y-2">
              {EXAMPLES.map((example, i) => (<button key={i} onClick={() => setInput(example)} className="w-full rounded-lg border border-x402-border bg-x402-surface px-4 py-3 text-left text-sm text-x402-text-secondary transition-all hover:border-x402-accent/30 hover:bg-x402-surface-hover hover:text-white">
                  {example}
                </button>))}
            </div>
          </Card_1.Card>
          ...
          {/* Advanced Options */}
          <Card_1.Card className="p-6">
            <button onClick={() => setShowAdvanced(!showAdvanced)} className="flex w-full items-center justify-between text-sm font-semibold text-white">
              Advanced Options
            ...
            {showAdvanced && (<div className="mt-6 space-y-6">
                <div>
                  ...
                  <input type="range" min="60" max="3600" step="60" value={maxTime} onChange={(e) => setMaxTime(Number(e.target.value))} className="w-full"/>
        ...
        <div className="space-y-6">
          ...
          <Card_1.Card className="p-6">
            ...
            <div className="mt-4">
              ...
              <input type="range" min="0.01" max={Math.min(10, balance.available || 0.1)} step="0.01" value={maxBudget} onChange={(e) => setMaxBudget(Number(e.target.value))} className="w-full"/>
              <div className="mt-1 flex justify-between text-xs text-x402-text-tertiary">
                <span>0.01 SOL</span>
            ...
            {balance.available < 0.01 && (<div className="mt-4 rounded-lg border border-x402-warning/20 bg-x402-warning/10 p-3 text-xs text-x402-warning">
          ...
          {/* Submit Button */}
          <Button_1.Button variant="primary" size="lg" onClick={handleSubmit} loading={loading} disabled={!input.trim() || charCount < charMin || charCount > charMax || !wallet} className="w-full">
