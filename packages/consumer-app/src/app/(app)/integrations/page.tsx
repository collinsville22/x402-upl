    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative flex-1">
          ...
          />
          <svg
            className="absolute left-3 top-3.5 h-5 w-5 text-x402-text-tertiary"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <select
          ...
          className="rounded-lg border border-x402-border bg-x402-surface px-4 py-3 text-sm text-white focus:border-x402-accent focus:outline-none focus:ring-1 focus:ring-x402-accent"
        >
            ...
            >
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {integration.capabilities.slice(0, 3).map((capability) => (
                  <span
                    key={capability}
                    className="rounded border border-x402-border bg-x402-black px-2 py-1 text-xs text-x402-text-secondary"
                  >
                    {capability}
                  </span>
                ))}
                {integration.capabilities.length > 3 && (
                  <span className="rounded border border-x402-border bg-x402-black px-2 py-1 text-xs text-x402-text-muted">
      ...
      {selectedIntegration && (
        ...
        >
          <Card
            className="max-w-2xl overflow-y-auto p-8"
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <button
                onClick={() => setSelectedIntegration(null)}
                className="text-x402-text-tertiary hover:text-white"
              >
            ...
            <div className="mt-6">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {selectedIntegration.capabilities.map((capability) => (
                  <span
                    key={capability}
                    className="rounded border border-x402-border bg-x402-black px-3 py-1 text-sm text-x402-text-secondary"
                  >
                    {capability}
                  </span>
                ))}
