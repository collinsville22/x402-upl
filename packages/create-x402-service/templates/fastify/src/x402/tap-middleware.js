...
export function createTAPMiddleware(config) {
    return async (request, reply, done) => {
        ...
        try {
            ...
            const did = request.headers['x-agent-did'];
            const cert = request.headers['x-agent-cert'];
            const wallet = request.headers['x-agent-wallet'];
            if (did)
                request.headers['x-tap-did'] = did;
            if (cert)
                request.headers['x-tap-cert'] = cert;
            if (wallet)
                request.headers['x-tap-wallet'] = wallet;
            done();
        }
        catch (error) {
...
function createSignatureBase(request, params) {
