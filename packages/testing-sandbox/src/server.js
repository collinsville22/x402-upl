...
async function buildServer() {
    ...
    const refundWallet = Keypair.fromSecretKey(Buffer.from(config.REFUND_WALLET_KEYPAIR, 'base64'));
    ...
    const paymentMiddleware = createX402Middleware({
        ...
        onPaymentVerified: async (paymentData) => {
            ...
            if (config.ENABLE_REFUNDS === 'true') {
                await refundManager.recordPayment(paymentData.signature, paymentData.from, parseFloat(paymentData.amount), paymentData.asset, paymentData.resource || 'unknown');
            }
        },
    });
