...
export class TaskDecomposer {
  ...
  private analyzeTask(description: string): Task[] {
  ...
  private async tasksToSteps(tasks: Task[]): Promise<ExecutionStep[]> {
  ...
  private canParallelize(task: Task, allTasks: Task[]): boolean {
    ...
    const samePriority = allTasks.filter(
      t => t.priority === task.priority && !task.dependencies.includes(t.id)
    );

    return samePriority.length > 0;
  }

  private optimizeExecution(steps: ExecutionStep[]): ExecutionStep[] {
  ...
  private calculateTotalTime(steps: ExecutionStep[]): number {
    const levelTimes = new Map<number, number>();

    const calculateLevel = (step: ExecutionStep, visited: Set<string> = new Set()): number => {
  ...
  private buildExecutionGraph(steps: ExecutionStep[]): string {
    const graph: string[] = [];
  ...
  private findCriticalPath(steps: ExecutionStep[]): string[] {
